...
class ParseFactsTests(unittest.TestCase):
    def test_ignores_echoed_placeholders_and_keeps_last_real_value(self) -> None:
        transcript = (
            b"printf 'SAVANTOS_FACT:yay:%s\\n' \"$(pacman -Q yay)\"\r\n"
            b"\x1b[?2004lSAVANTOS_FACT:yay:missing\r\n"
            b"SAVANTOS_FACT:sshd:inactive\r\n"
            b"SAVANTOS_FACT:yay:present\r\n"
        )
    ...
    def test_tolerates_bad_utf8_and_rejects_quoted_or_escaped_values(self) -> None:
        transcript = (
            b"\xffSAVANTOS_FACT:foreign:0\n"
            b"SAVANTOS_FACT:quoted:'present'\n"
            b"SAVANTOS_FACT:escaped:present\\later\n"
        )
