...
- ARM64 Windows PCs (Snapdragon and similar) are not supported: the launcher,
...
- The launcher boots its pinned kernel and initramfs from the release image,
  and the guest's pacman configuration holds the `linux` package so
  `pacman -Syu` and `omarchy-update` leave it alone. Kernel updates arrive
  with guest-image updates, which also carry the matching modules onto
